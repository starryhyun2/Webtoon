...
public class MyPageController {
	...
	@RequestMapping("/")						// 기본화면 
	public String home() {						// 회원가입 페이지를 홈으로 설정
		return "/WEB-INF/views/memberForm.jsp";
	}
	
	@RequestMapping("member.do")				// 회원가입 페이지에서 정보들을 입력후 '회원가입' 버튼을 누르면 실행되는 메서드
	public String member(Webtoon_UserVO vo) {	// 회원가입 페이지에서 받아온 정보들을 Webtoon_UserVO에 저장
		
		int res = user_dao.insert(vo);			// user_dao의 modify메소드에게 저장된 vo객체를 넘겨 DB에 정보 저장
		
		return "redirect:login.do";				// 회원가입 후 바로 로그인 페이지로 넘어가기
	}
	
	@RequestMapping("/login.do")				// /login.do가 실행되면 loginform.jsp 화면 출력
	public String login() {
