...
<body>
	...
	<input id="mypage_btn" type="button" value="My 페이지" onclick="location.href='Mypage'">
...
<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
  <li class="nav-item" role="presentation">
    <button class="nav-link active" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-1" type="button" role="tab" aria-controls="pills-home" aria-selected="true">액션</button>
  </li>
  <li class="nav-item" role="presentation">
			...
			<div class="col-4 mt_box">
				...
				<!-- 첨부된 이미지가 있는 경우에만 img태그를 보여주자! -->
					...
					<a href="show?idx=${ mt.webtoon_idx }">
		      		...
		      		<div class="type_name box-con"><p>작성자 : <span>${ mt.author }</span></p></div>
		      		<div class="type_title box-con"><p>제목 : <span>${ mt.title }</span></p></div>
		      		<div class="type_regdate box-con"><p>작성일 : <span>${fn:split(mt.regdate,' ')[0]}</span></p></div>
		    	</div>
	    	</div>
      	</c:if>
	</c:forEach>
...
<c:forEach var="mt" items="${mt_list}">
		...
		<!-- 첨부된 이미지가 있는 경우에만 img태그를 보여주자! -->
			<c:if test="${mt.filename ne 'no_file'}">
      		...
      		<div class="type_content box-con"><p>${ mt.info }</p></div>	
      		<div class="type_name box-con"><p>작성자 : <span>${ mt.author }</span></p></div>
      		<div class="type_name box-con"><p>제목 : <span>${ mt.title }</span></p></div>
      		<div class="type_regdate box-con"><p>작성일 : <span>${fn:split(mt.regdate,' ')[0]}</span></p></div>
      		</div>
    	</div>
      	</c:if>
